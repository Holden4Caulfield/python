...
def getCover(html,soup):
#获取封面信息	
	global tag
	items=soup('a',attrs={'class':'movie-box'})
	count=0
	try:
		for item in items:
			if(tag==1):
				...
				break
			count=count+1
			if(item.find('button',attrs={"title":"包含最新出種的磁力連結"}).string!='今日新種'):
				count
				break
			else:	
				filename=item('date')[0].string+' '+item('date')[1].string+'.jpg'
				pic_url=item.find('img')['src']
				#print(pic_url)
				saveCover(pic_url,filename)
				print('\r种子封面当前进度:{:2f}%\r'.format(count*100/len(items),end=''))
	except:
		print('error')
...
def main():
	...
	page=1
	global tag
	if os.path.exists(date_root+date+'.txt'):
		tag=1
	while(today_task):
